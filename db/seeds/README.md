...
| `user-preferences.json` | Dev roster keyed by email (`is_dev: true`) so the assignee dropdown and Schedule cards populate. Includes the local viewer (`local@comley-nexus`) so their profile/stats page works out of the box. |
