...
// Insert any platform template that isn't in the table yet.
//
...
// anyone opened it. Every template added afterwards needed hand-written SQL
// to appear — exactly the drift this module claims to prevent. So it diffs
// by id instead: same one round-trip, but a new entry in the array shows up
// on the next read.
//
// Existing rows are never rewritten. An admin's edits and their is_active
// flag survive, which is also why deactivating is the way to hide a platform
// template — a hard-deleted row is treated as missing and comes back.
async function ensureSeeded() {
  ...
  // ignoreDuplicates keeps concurrent first-reads from colliding on the id.
