...
// that grant was recorded and never consulted by CMS.
//
...
// lib/coreFlags.js and lib/coreBilling.js, which do the same for
// feature_flags / billing_accounts against the same Core project.
//
// Best-effort / non-fatal: callers only reach these after this app's own
...
// workspace" selector (unlike Field's X-Workspace header — see lib/auth.js's
// loadOrg). That means an app-wide (org_id null) grant can never be resolved
// here: there's no signal telling us which org to open. This only surfaces
// an unambiguous org-SCOPED grant — i.e. exactly one non-null-org_id row for
// this user+app — as their default org. Two or more scoped grants, or only
// an app-wide grant, resolve to null (documented gap, not a bug).
export async function getDefaultAppAccessOrg(clerkUserId) {
  if (!core || !clerkUserId) return null;
  try {
    const { data, error } = await core.from('app_access').select('org_id, role')
      .eq('clerk_user_id', clerkUserId).eq('app_key', APP_KEY).not('org_id', 'is', null);
    if (error) throw error;
    return (data && data.length === 1) ? data[0] : null;
  } catch (e) {
    console.error('[coreAppAccess] getDefaultAppAccessOrg failed (non-fatal):', e.message);
    return null;
  }
}
