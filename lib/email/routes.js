...
// Newsletter is a standard CMS feature — available to every workspace,
// independent of Commerce or any paid flag (writes are still role-gated, and
// real sending only happens when Resend is configured; otherwise it
// sandboxes). Kept as a named middleware so a gate can return later if needed.
const requireEmail = (req, res, next) => next();
...
export function mountEmailApi(app) {
  ...
  app.get('/api/email/status', requireOrg, (req, res) => {
    res.json({ enabled: true, sandbox: sandbox(), aiConfigured: ai.hasAI(), sendSandbox: sender.isSandbox() });
  });
  ...
  app.post('/api/email/templates', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
    try { res.json(await templates.save(req.org.id, { ...req.body, createdBy: req.viewer.email })); } catch (e) { next(e); }
  });
  app.delete('/api/email/templates/:id', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
  ...
  app.post('/api/email/ai/generate', requireOrg, requireEmail, requirePermission('email', 'edit'), aiLimit, async (req, res, next) => {
    try {
  ...
  app.post('/api/email/ai/copy', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
    try { res.json(await ai.suggestCopy({ brief: req.body?.brief, tone: req.body?.tone })); } catch (e) { next(e); }
  });
  app.post('/api/email/ai/restyle', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
  ...
  app.post('/api/email/campaigns', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
    try { res.json(await campaigns.save(req.org.id, { ...req.body, createdBy: req.viewer.email })); } catch (e) { next(e); }
  });
  app.delete('/api/email/campaigns/:id', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
  ...
  app.post('/api/email/campaigns/:id/send', requireOrg, requireEmail, requirePermission('email', 'edit', 'send'), async (req, res, next) => {
    try {
      const c = await campaigns.get(req.org.id, req.params.id);
      if (!c) return res.status(404).json({ error: 'Campaign not found' });
  ...
  app.post('/api/email/campaigns/:id/test', requireOrg, requireEmail, requirePermission('email', 'edit'), async (req, res, next) => {
    try {
