...
import { getDefaultAppAccessOrg } from './coreAppAccess.js';
...
//
// Value is JSON `{ org, role }` (role optional, defaults to 'admin') --
// matches nexus-field's `view_as` cookie shape, so both apps' view-as
// impersonation can carry a role, not just hardcode admin. Set either by
// CMS's own POST /api/super-admin/view-as/:orgId (lib/superAdminRoutes.js)
// or by exchanging a Nexus Command-issued handoff token at
// POST /api/view-as/accept (lib/viewAsHandoff.js).
...
// Fallback identity source #2, tried only after loadSharedViewer (below, in
// resolveViewer) comes up empty: this app's own long-lived `shared_session`
// cookie, self-issued via lib/sessionHandoff.js after redeeming a
// Command-minted SESSION handoff token (see that file's header).
// loadSharedViewer only recognizes a LIVE shared-Clerk credential — a
// Bearer header, or the shared instance's own short-lived __session cookie,
// which can't even be set cross-domain from Command's origin to CMS's
// separate root domain anyway. This cookie is what makes that sign-in LAST
// beyond the ~60s a copied live token would, with no refresh mechanism on
// this side. Returns the same shape loadSharedViewer produces, minus
// name/image (the cookie carries only sub+email) — downstream code already
// handles a viewer with just userId/email.
function loadSharedSessionCookie(req) {
  const payload = verifySharedSessionCookie(req.cookies?.[SHARED_SESSION_COOKIE]);
  if (!payload) return null;
  return { userId: payload.sub, email: payload.email, name: payload.email, image: null };
}
...
async function loadOrg(req, viewer) {
  ...
  const raw = req.cookies?.[VIEW_AS_COOKIE];
  if (raw && isSuperAdminViewer(viewer)) {
    // Cookie value is JSON ({org, role}), matching Field's shape — see
    // VIEW_AS_COOKIE's own comment. Older cookies pre-dating that (set
    // before this change) are a bare org id string; those still work,
    // falling back to 'admin' below, and simply expire within their
    // original 4h maxAge like any other cookie.
    let orgId, role;
    try { ({ org: orgId, role } = JSON.parse(raw)); } catch { orgId = raw; }
    if (orgId) {
      const org = await orgsStorage.get(orgId);
      if (org) {
        const effectiveRole = ROLE_RANK.hasOwnProperty(role) ? role : 'admin';
        return { id: org.id, slug: org.id, name: org.name, role: effectiveRole, feature_flags: org.feature_flags || {}, domain: org.domain || null, paused: !!org.paused, viewingAs: true };
      }
    }
  }
  const own = await orgForUser(viewer.email);
  if (own) return own;
  // Shared-Clerk fallback (lib/dualAuth.js) with no org_members row of its
  // own: consult Nexus Command's app_access grants
  // (nexus-core/packages/core/db/migrations/005_app_access.sql,
  // lib/coreAppAccess.js). Only reachable when the viewer came in through
  // the shared instance (see resolveViewer's `_sharedInstance` flag) — this
  // app's own primary-Clerk-instance login path never reaches here.
  //
  // CMS resolves exactly one org per session with no per-request "which
  // workspace" selector (unlike Field's X-Workspace header) — RequireOrg.jsx
  // just redirects to whatever org the server already picked. So only an
  // unambiguous org-SCOPED grant can be resolved here; an app-wide
  // (org_id null) grant has no signal telling us which org to open and is
  // not resolved by this path — documented gap, see
  // getDefaultAppAccessOrg's own comment.
  if (viewer._sharedInstance) {
    const grant = await getDefaultAppAccessOrg(viewer.userId);
    if (grant) {
      const org = await orgsStorage.get(grant.org_id);
      if (org) {
        return { id: org.id, slug: org.id, name: org.name, role: grant.role, feature_flags: org.feature_flags || {}, domain: org.domain || null, paused: !!org.paused };
      }
    }
  }
  return null;
...
export function resolveViewer(req, _res, next) {
  ...
  // Identity source precedence: (1) this app's own Clerk instance session,
  // checked above via `userId`; (2) a LIVE shared-Clerk credential (Phase C
  // pilot, lib/dualAuth.js's loadSharedViewer — a Bearer token or the
  // shared instance's own short-lived __session cookie); (3) this app's own
  // long-lived `shared_session` cookie (lib/sessionHandoff.js /
  // loadSharedSessionCookie above), self-issued after redeeming a Command
  // SESSION handoff — never the reverse, and an own-instance session always
  // wins over both shared-instance sources when present.
  loadSharedViewer(req).then((shared) => {
    if (shared) {
      const bootstrapAdmin = adminEmails().includes(shared.email.toLowerCase());
      return finishResolve({ ...shared, role: bootstrapAdmin ? 'admin' : 'viewer', _sharedInstance: true });
    }
    const cookieViewer = loadSharedSessionCookie(req);
    if (cookieViewer) {
      const bootstrapAdmin = adminEmails().includes(cookieViewer.email.toLowerCase());
      return finishResolve({ ...cookieViewer, role: bootstrapAdmin ? 'admin' : 'viewer', _sharedInstance: true });
    }
    return denyAuth();
  }).catch(denyAuth);
}
