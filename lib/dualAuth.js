...
export async function loadSharedViewer(req) {
  ...
  try {
    ...
    // TEMP diagnostic (not a behavior change -- still returns null either
    // way): this catch was silent, making a real live failure
    // indistinguishable from "no token." Logging to find out which one
