...
// gated by requirePermission('events', 'edit') like page/content editing. Read routes are
...
export function mountEventsApi(app) {
  ...
  app.post('/api/calendars', requireOrg, requirePermission('events', 'edit'), async (req, res, next) => {
    try { res.json({ calendar: await calendars.create(req.org.id, req.body || {}) }); } catch (e) { next(e); }
  });
  app.patch('/api/calendars/:id', requireOrg, requirePermission('events', 'edit'), async (req, res, next) => {
    try {
  ...
  app.delete('/api/calendars/:id', requireOrg, requirePermission('events', 'edit'), async (req, res, next) => {
  ...
  app.post('/api/events', requireOrg, requirePermission('events', 'edit'), async (req, res, next) => {
    try { res.json({ event: await events.create(req.org.id, req.body || {}) }); } catch (e) { next(e); }
  });
  app.patch('/api/events/:id', requireOrg, requirePermission('events', 'edit'), async (req, res, next) => {
    try {
  ...
  app.delete('/api/events/:id', requireOrg, requirePermission('events', 'edit'), async (req, res, next) => {
