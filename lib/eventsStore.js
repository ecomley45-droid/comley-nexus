...
const EVENT_COLS = 'id, calendar_id, title, description, location, starts_at, ends_at, all_day, flyer_url, link_url, recurrence, recurrence_until';
...
function eventRow(r) {
  return {
    ...
    recurrence: r.recurrence || 'none', recurrenceUntil: r.recurrence_until,
  };
}
...
export const events = {
  ...
  async list(orgId, { calendarId } = {}) {
    let q = db().from('events').select(EVENT_COLS).eq('org_id', orgId);
  ...
  async create(orgId, e) {
    const { data, error } = await db().from('events').insert({
      ...
      recurrence: e.recurrence || 'none', recurrence_until: e.recurrenceUntil || null,
  ...
  async update(orgId, id, patch) {
    const map = { calendarId: 'calendar_id', title: 'title', description: 'description', location: 'location',
      ...
      recurrence: 'recurrence', recurrenceUntil: 'recurrence_until' };
    const cols = { updated_at: new Date().toISOString() };
    ...
    const { data, error } = await db().from('events').update(cols)
      .eq('org_id', orgId).eq('id', id).select(EVENT_COLS).maybeSingle();
