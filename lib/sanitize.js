...
const CONTENT_CONFIG = {
  ...
  // iframe explicitly excludes 'data' -- a data: URI can smuggle a full
  // inline HTML document, same risk class as srcdoc (which is also not in
  // its allowedAttributes above).
...
export function sanitizeGlobalSettings(gs) {
  ...
  const theme = gs.theme && typeof gs.theme === 'object'
    ? { ...gs.theme, customCss: sanitizeCss(gs.theme.customCss || '') }
    : gs.theme;
