...
import { normalizeSectionStyle } from '../src/shared/blockStyle.js';
import { normalizeLocales } from '../src/shared/i18n.js';

// Inline SVG.
//
// Stripping it was quietly costing real design: modern documents carry their
// icons, logos and decorative marks as inline <svg>, and every one of them
// vanished on import with nothing to show for it.
//
// Names are lowercase because the parser lowercases them, and that is fine:
// when a browser parses inline SVG in an HTML document it runs the spec's
// "adjust SVG tag names" and "adjust SVG attributes" tables, so `lineargradient`
// becomes linearGradient and `viewbox` becomes viewBox on the way into the
// DOM. Verified rather than assumed -- an all-lowercase gradient/clipPath
// SVG parses back as SVGGradientElement with a 24x24 viewBox and renders.
//
// What is deliberately NOT here is the dangerous half of SVG:
//
//   script, handler            executable by definition
//   foreignObject              an escape hatch back into arbitrary HTML
//   animate, animateTransform,
//   animateMotion, set         SMIL can retarget an href to javascript: after
//                              the sanitizer has already passed the markup
//
// <use> stays, but transformTags below pins it to same-document fragments:
// a `use` pointing at an external file is a content-exfiltration primitive.
const SVG_TAGS = [
...
const SVG_ATTRS = [
  ...
  'width', 'height', 'd', 'points', 'transform', 'offset', 'href', 'xlink:href',
  ...
  'patternunits', 'patterncontentunits', 'clippathunits', 'maskunits', 'maskcontentunits',
  'markerwidth', 'markerheight', 'markerunits', 'orient', 'refx', 'refy',
  'filterunits', 'primitiveunits', 'stddeviation', 'in', 'in2', 'result', 'mode',
  'type', 'values', 'flood-color', 'flood-opacity', 'dx', 'dy',
  'font-family', 'font-size', 'font-weight', 'font-style', 'letter-spacing',
  'text-anchor', 'dominant-baseline', 'alignment-baseline', 'paint-order',
  'vector-effect', 'shape-rendering', 'overflow', 'pathlength', 'startoffset',
];
...
const CONTENT_CONFIG = {
  allowedTags: [
    ...
    'script', 'noscript',
    // <canvas>: a scriptable drawing surface with no inherent capability of
    // its own (the power lives in the already-allowed <script>). Needed for
    // JS-driven hero animations/visualizations on full-HTML pages, which were
    // silently broken because the canvas element got stripped while its
    // driving script survived.
    'canvas',
  ],
  allowedAttributes: {
    '*': ['class', 'id', 'style', 'data-*', 'aria-*', 'role', 'title', 'lang', ...SVG_ATTRS],
  ...
  transformTags: {
    ...
    // <use> may only reference a fragment in the same document. Pointed at a
    // URL it fetches that document, which is a way to pull content the
    // visitor never asked for -- and historically a way to smuggle script.
    use: (tagName, attribs) => {
      for (const key of ['href', 'xlink:href']) {
        if (attribs[key] && !attribs[key].startsWith('#')) delete attribs[key];
      }
      return { tagName, attribs };
    },
  },
...
const FULL_PAGE_CONFIG = {
  ...
  allowedAttributes: {
    ...
    // Event handlers survive on full-HTML pages, and ONLY there.
    //
    // Full HTML mode is the admin-only "paste a whole document, we serve it
    // as-is" escape hatch, and its CSP already allows inline <script>
    // (unsafe-inline). So stripping onclick/onsubmit/etc. here blocks nothing
    // an author couldn't do with a <script> tag anyway -- it just silently
    // breaks pasted interactive HTML. The classic symptom: a card's
    // onclick="openModal('x')" vanishes on save while the openModal function
    // in the page's <script> stays, so the button is wired to nothing and the
    // card never opens.
    //
    // Block content keeps them stripped (CONTENT_CONFIG is unchanged): its
    // strict CSP has no unsafe-inline so a handler wouldn't fire anyway, and
    // its editors are a lower trust tier than the admin who owns full-HTML.
    '*': [...(CONTENT_CONFIG.allowedAttributes['*'] || []), 'on*'],
    html: ['lang'],
    body: ['class', 'style'],
...
// Drop <script> that sits inside an <svg>.
//
// `script` is allowed globally, for the Script block, so this closes nothing
// on its own -- a page that wants inline JS can still carry a plain <script>,
// and that path has its own admin-role gate. What it does do is keep the
// newly-allowed SVG from becoming a second, less obvious carrier for the same
// thing: no icon or logo has ever needed a script tag inside it, so there is
// no cost to refusing one.
//
// Runs after sanitize-html, on markup it has already normalised (quoted
// attributes, closed tags), which is what makes depth counting reliable here.
// Two tags mean different things inside an <svg> than outside it, and
// sanitize-html's allowlist is flat -- it has no notion of "only in this
// parent". This pass adds that one distinction.
//
//   script  dropped INSIDE svg. `script` is allowed globally for the Script
//           block, so this closes nothing on its own; it just stops the
//           newly-allowed SVG becoming a second, less obvious carrier. No
//           icon has ever needed one.
//
//   title   dropped OUTSIDE svg in content mode. <title> inside an <svg> is
//           its accessible name, which is worth keeping; <title> loose in a
//           block is a document-skeleton tag that would set the page title
//           from inside a section.
//
// Runs on markup sanitize-html has already normalised -- quoted attributes,
// closed tags -- which is what makes depth counting reliable.
function scopeSvgTags(html, dropOutside = []) {
  if (!html.includes('<svg')) {
    return dropOutside.length ? dropElements(html, dropOutside) : html;
  }
  const names = ['svg', 'script', ...dropOutside].join('|');
  const TAG = new RegExp(`<(/?)(${names})\\b[^>]*>`, 'gi');
  let out = '';
  let depth = 0;
  let i = 0;
  let m;
  while ((m = TAG.exec(html))) {
    const [tag, closing, raw] = m;
    const name = raw.toLowerCase();
    if (name === 'svg') {
      out += html.slice(i, m.index) + tag;
      depth = Math.max(0, depth + (closing ? -1 : 1));
      i = m.index + tag.length;
      continue;
    }
    const unwanted = closing ? false
      : (name === 'script' ? depth > 0 : depth === 0 && dropOutside.includes(name));
    if (!unwanted) continue;
    const close = `</${name}>`;
    const end = html.toLowerCase().indexOf(close, m.index);
    out += html.slice(i, m.index);
    i = end === -1 ? html.length : end + close.length;
    TAG.lastIndex = i;
  }
  return out + html.slice(i);
}

// Same removal, for documents with no <svg> at all.
...
export function sanitizeContentHtml(html) {
  ...
  return scopeSvgTags(sanitizeHtml(html, CONTENT_CONFIG), ['title']);
}

export function sanitizeFullPageHtml(html) {
  if (typeof html !== 'string') return '';
  // A full page legitimately has a document <title>, so only script is scoped.
  return scopeSvgTags(sanitizeHtml(html, FULL_PAGE_CONFIG));
}
...
export function sanitizePage(page) {
  ...
  const content = Array.isArray(page.content)
    ? page.content.map((section) => {
        ...
        // Design tokens aren't HTML, but they are interpolated straight into
        // a <style> block by buildPageStyleCss at compile time — so they get
        // the same treatment: an allowlisted, range-checked rewrite that
        // drops anything unrecognized rather than trusting the client.
        const style = normalizeSectionStyle(section.style);
        if (style) clean.style = style; else delete clean.style;
        return clean;
      })
    : page.content;
  ...
  // A draft holds the same shapes as the live page and gets published as-is,
  // so it goes through exactly the same cleaning. Sanitising only on publish
  // would leave unsanitised markup sitting in the database in the meantime.
  const draft = page.draft && typeof page.draft === 'object'
    ? sanitizePage({ ...page.draft, draft: undefined })
    : page.draft;
  if (draft) delete draft.draft;

  return { ...page, content, layout, analytics, fullHtml, ...(draft ? { draft } : {}) };
}
...
export function sanitizeGlobalSettings(gs) {
  ...
  const theme = gs.theme && typeof gs.theme === 'object'
    ...
    : gs.theme;
  // Saved design presets are just named style-token objects, so they go
  // through the same normalizer a section's style does -- a preset is applied
  // straight onto a block, so anything unsafe here would land there.
  const designPresets = Array.isArray(gs.designPresets)
    ? gs.designPresets
        .filter((p) => p && typeof p.name === 'string')
        .slice(0, 40)
        .map((p) => ({ name: p.name.slice(0, 60), style: normalizeSectionStyle(p.style) }))
        .filter((p) => p.style)
    : gs.designPresets;

  // Locale codes become URL segments and hreflang values, so they're
  // validated to a BCP-47-ish shape rather than trusted.
  const locales = gs.locales === undefined ? gs.locales : normalizeLocales(gs.locales);

  return { ...gs, theme, globals, analytics, designPresets, ...(locales ? { locales } : {}) };
}
