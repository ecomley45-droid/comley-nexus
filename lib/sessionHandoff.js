...
// separate task on Command's side) for CMS's own long-lived
...
//   - view-as/accept lets a Command super-admin IMPERSONATE a CMS
//     workspace (grants an org + role via the VIEW_AS_COOKIE, no real
...
//     still decided entirely by resolveViewer's existing org_members /
...
// can't write a cookie onto CMS's origin directly. Instead it hands the
// operator's OWN browser a same-origin top-level POST to make here (not a
// fetch/XHR, so no CORS involved at all), and CMS sets its own cookie on
// its own origin.
...
// top-level navigation, not fetch/XHR, so the app-wide express.json() never
// sees it.
...
export function mountSessionHandoff(app) {
  ...
  // src/main.jsx's <ClerkProvider afterSignOutUrl>) so signing out of CMS
  // also drops any shared-session identity, not just an own-instance Clerk
  // session.
