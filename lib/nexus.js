...
// Nexus's own media library. Mirrors lib/storage.js's `media` helpers but,
// like everything else here, takes no orgId and hits the standalone
// `nexus_media` table. The bytes live in the shared `media` Storage bucket
// under a `nexus/` folder (see the /api/nexus/media routes in server.js).
export const media = {
  async list() {
    const run = () => db().from('nexus_media')
      .select(mediaCols('nexus_media'))
      .order('uploaded_at', { ascending: false });
    let { data, error } = await run();
    if (error && mediaHasVariants('nexus_media') && isMissingVariantColumn(error)) {
      markVariantsMissing('nexus_media');
  ...
  async add(entry) {
    ...
    let { error } = await db().from('nexus_media').insert(mediaHasVariants('nexus_media') ? withVariants() : base);
    if (error && mediaHasVariants('nexus_media') && isMissingVariantColumn(error)) {
      markVariantsMissing('nexus_media');
      ({ error } = await db().from('nexus_media').insert(base));
    }
    throwOn('media.add', error);
    return entry;
  },
  // Only the three user-editable fields can be patched; the storage-managed
  // columns (filename, url, mime_type, size) are never touched by an edit.
  async update(id, patch) {
    ...
    const { data, error } = await db().from('nexus_media')
      .update(columns).eq('id', id)
      .select(mediaCols('nexus_media'))
      .maybeSingle();
