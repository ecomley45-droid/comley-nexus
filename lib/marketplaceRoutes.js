...
import { requirePermission, isSuperAdminViewer } from './auth.js';
...
export function mountMarketplaceApi(app) {
  ...
  // Export one template as a portable file, and import one back.
  //
  // This exists because a template had exactly one copy — a row in one
  // database — and a bad DELETE was therefore unrecoverable. A file on disk
  // is a backup that survives the database entirely, and doubles as the way
  // to move a template between environments.
  //
  // Export is available to anyone who can see the template; it returns what
  // the marketplace already shows them, in a form they can keep.
  app.get('/api/templates/:id/export', requireAuth, async (req, res, next) => {
    try {
      const t = await templates.get(req.params.id);
      if (!t) return res.status(404).json({ error: 'Template not found' });
      // Same visibility rule as GET /api/templates/:id — a workspace-private
      // template is only visible to its own org, and "not visible" is a 404
      // rather than a 403 so the endpoint can't be used to prove one exists.
      ...
      const file = {
        ...
        description: t.description, featureList: t.featureList,
        payload: t.payload,
      };
      const safe = String(t.slug || t.id).replace(/[^a-z0-9-]/gi, '-').slice(0, 60);
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Content-Disposition', `attachment; filename="${safe}.nexus-template.json"`);
      res.send(JSON.stringify(file, null, 2));
    } catch (e) { next(e); }
  });

  app.post('/api/templates/import', requireSuperAdmin, async (req, res, next) => {
    try {
      const file = req.body || {};
      if (file.nexusTemplate !== 1) {
        return res.status(400).json({ error: 'This is not a Nexus template file.' });
      }
      if (!file.name?.trim()) return res.status(400).json({ error: 'The file has no template name.' });
      // The payload goes through the same validation as any other write —
      // a file off disk is untrusted input, however it was produced.
      const payload = validateSitePayload(file.payload);
      if (payload.pages.length === 0) {
        return res.status(400).json({ error: 'The file contains no valid pages.' });
      }
      const id = `tpl-${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
      const created = await templates.create({
        id, orgId: null,
        slug: String(file.slug || '').trim() || id,
        name: String(file.name).trim().slice(0, 80),
        category: String(file.category || 'Business').trim(),
        description: String(file.description || ''),
        featureList: Array.isArray(file.featureList) ? file.featureList : [],
        payload,
      });
      res.json({ success: true, template: created, summary: summarizePayload(payload) });
    } catch (e) { next(e); }
  });
  ...
  app.post('/api/templates/:id/install', requireOrg, requirePermission('templates', 'edit', 'install'), async (req, res, next) => {
    try {
      const t = await templates.get(req.params.id);
      if (!t || !t.isActive) return res.status(404).json({ error: 'Template not found' });
  ...
  app.get('/api/backups', requireOrg, requirePermission('backups', 'view'), async (req, res, next) => {
    try {
      res.json({ backups: await backups.list(req.org.id) });
    } catch (e) { next(e); }
  });

  app.post('/api/backups', requireOrg, requirePermission('backups', 'edit'), async (req, res, next) => {
    try {
  ...
  app.post('/api/backups/:id/restore', requireOrg, requirePermission('backups', 'edit'), async (req, res, next) => {
    try {
  ...
  app.delete('/api/backups/:id', requireOrg, requirePermission('backups', 'edit'), async (req, res, next) => {
    try {
