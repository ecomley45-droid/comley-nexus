...
export function registerMediaRoutes(app, ctx) {
  const { storage, requireOrg, requireRole, requireSuperAdmin, auditFor, express } = ctx;
  ...
  app.post('/api/media', express.json({ limit: '44mb' }), requireOrg, requireRole('editor'), async (req, res, next) => {
    try {
