...
import { isSystemRole } from '../../src/shared/permissions.js';
...
export function registerTeamRoutes(app, ctx) {
  ...
  app.post('/api/team', requireOrg, requirePermission('team', 'edit'), async (req, res, next) => {
    try {
      const { name, email, role } = req.body;
      if (!name?.trim() || !email?.trim() || !role?.trim()) {
        return res.status(400).json({ error: 'name, email, and a valid role are required' });
      }
      // Role must be a system role or a custom role defined in THIS workspace.
      const validRole = isSystemRole(role) || !!(await storage.roles.get(req.org.id, role).catch(() => null));
      if (!validRole) {
        return res.status(400).json({ error: `Unknown role "${role}" for this workspace` });
      }
  ...
  app.delete('/api/team/:id', requireOrg, requirePermission('team', 'edit'), async (req, res, next) => {
    try {
