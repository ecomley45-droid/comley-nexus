...
async function fulfillOrder({ items, customer, campaignCode, paymentIntentId }) {
  ...
  if (campaignCode) await campaignsRepo.recordCampaignUsage(campaignCode, total);
  ...
  app.post('/api/webhooks/stripe', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    try {
      ...
      if (event.type === 'checkout.session.completed' && event.data.object.metadata?.commerce_item) {
        ...
        const customer = email ? await customersRepo.getCustomerByEmail(email) : null;
        await fulfillOrder({
          items: [item],
      ...
      if (event.type === 'payment_intent.succeeded') {
        ...
        const customer = pi.metadata.customerEmail
          ? await customersRepo.getCustomerByEmail(pi.metadata.customerEmail)
          : null;
        await fulfillOrder({
          items,
          customer,
        ...
        if (order) {
          await ordersRepo.updateOrderStatus(order.id, 'refunded');
...
export function mountCommerceApi(app) {
  ...
  router.get('/products', async (req, res) => {
    res.json(await productsRepo.listProducts());
  });
  ...
  router.post('/products', requireCommerceTier('admin'), async (req, res) => {
    const product = await productsRepo.createProduct(req.body);
    await upsertProductVector(product);
    res.json({ success: true, product });
  });

  router.put('/products/:id', requireCommerceTier('admin'), async (req, res) => {
    const product = await productsRepo.updateProduct(req.params.id, req.body);
    if (!product) return res.status(404).json({ error: 'Product not found' });
    await upsertProductVector(product);
    res.json({ success: true, product });
  });

  router.delete('/products/:id', requireCommerceTier('admin'), async (req, res) => {
    const removed = await productsRepo.deleteProduct(req.params.id);
  ...
  router.get('/orders', requireCommerceTier('admin'), async (req, res) => {
    res.json(await ordersRepo.listOrders());
  });
  ...
  router.post('/orders/:id/status', requireCommerceTier('admin'), async (req, res) => {
    const { status } = req.body;
    if (!['pending', 'paid', 'refunded', 'cancelled'].includes(status)) {
      return res.status(400).json({ error: 'Invalid status' });
    }
    const order = await ordersRepo.updateOrderStatus(req.params.id, status);
    if (!order) return res.status(404).json({ error: 'Order not found' });
  ...
  router.get('/customers', requireCommerceTier('admin'), async (req, res) => {
    res.json(await customersRepo.listCustomers());
  });

  router.patch('/customers/:id/tier', requireCommerceTier('admin'), async (req, res) => {
    const { tier } = req.body;
    if (!['customer', 'wholesaler', 'admin'].includes(tier)) {
      return res.status(400).json({ error: 'Invalid tier' });
    }
    const customer = await customersRepo.updateCustomerTier(req.params.id, tier);
    if (!customer) return res.status(404).json({ error: 'Customer not found' });
    res.json({ success: true, customer });
  });

  // ---- Discounts / campaigns (admin dashboard) ----
  router.get('/campaigns', requireCommerceTier('admin'), async (req, res) => {
    res.json(await campaignsRepo.listCampaigns());
  });

  router.post('/campaigns', requireCommerceTier('admin'), async (req, res) => {
    try {
  ...
  router.put('/campaigns/:code', requireCommerceTier('admin'), async (req, res) => {
    const campaign = await campaignsRepo.updateCampaign(req.params.code, req.body);
    if (!campaign) return res.status(404).json({ error: 'Campaign not found' });
    res.json({ success: true, campaign });
  });

  router.delete('/campaigns/:code', requireCommerceTier('admin'), async (req, res) => {
    const removed = await campaignsRepo.deleteCampaign(req.params.code);
  ...
  router.get('/export/csv/products', requireCommerceTier('admin'), async (req, res) => {
    csvAttachment(res, 'products.csv', rowsToCsv(await productsRepo.listProducts(), PRODUCT_CSV_COLUMNS));
  });
  ...
  router.post('/import/csv/products', requireCommerceTier('admin'), async (req, res) => {
    const { csv } = req.body;
    ...
    for (const [idx, row] of csvToRows(csv).entries()) {
      try {
        ...
        if (match) { await productsRepo.updateProduct(match.id, patch); updated++; }
        else { await productsRepo.createProduct(patch); created++; }
      } catch (err) {
  ...
  router.get('/export/csv/orders', requireCommerceTier('admin'), async (req, res) => {
    const rows = (await ordersRepo.listOrders()).map((o) => ({ ...o, items_json: JSON.stringify(o.items) }));
  ...
  router.post('/import/csv/orders', requireCommerceTier('admin'), async (req, res) => {
    const { csv } = req.body;
    if (typeof csv !== 'string') return res.status(400).json({ error: 'csv (string) is required' });
    let created = 0, updated = 0;
    const errors = [];
    for (const [idx, row] of csvToRows(csv).entries()) {
      try {
        if (row.id) {
          ...
          if (existing) {
            await ordersRepo.updateOrderStatus(row.id, row.status || existing.status);
            updated++;
            continue;
          }
        }
        ...
        await ordersRepo.createOrder({
          customer_email: row.customer_email || null, items,
  ...
  router.get('/export/csv/customers', requireCommerceTier('admin'), async (req, res) => {
    csvAttachment(res, 'customers.csv', rowsToCsv(await customersRepo.listCustomers(), CUSTOMER_CSV_COLUMNS));
  });
  ...
  router.post('/import/csv/customers', requireCommerceTier('admin'), async (req, res) => {
    const { csv } = req.body;
    if (typeof csv !== 'string') return res.status(400).json({ error: 'csv (string) is required' });
    const existing = await customersRepo.listCustomers();
    let created = 0, updated = 0;
    const errors = [];
    for (const [idx, row] of csvToRows(csv).entries()) {
      try {
        ...
        await customersRepo.upsertCustomer({ clerkId, email: row.email || '', tier: row.tier || 'customer' });
  ...
  router.get('/export/csv/campaigns', requireCommerceTier('admin'), async (req, res) => {
    csvAttachment(res, 'discounts.csv', rowsToCsv(await campaignsRepo.listCampaigns(), CAMPAIGN_CSV_COLUMNS));
  });
  ...
  router.post('/import/csv/campaigns', requireCommerceTier('admin'), async (req, res) => {
    const { csv } = req.body;
    if (typeof csv !== 'string') return res.status(400).json({ error: 'csv (string) is required' });
    let created = 0, updated = 0;
    const errors = [];
    for (const [idx, row] of csvToRows(csv).entries()) {
      try {
        ...
        const existing = await campaignsRepo.getCampaignByCode(code);
        if (existing) { await campaignsRepo.updateCampaign(code, patch); updated++; }
        else { await campaignsRepo.createCampaign({ code, ...patch }); created++; }
      } catch (err) {
        errors.push({ row: idx + 2, message: err.message });
      }
    }
