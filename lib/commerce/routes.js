...
import * as staffRepo from './staffRepo.js';
import * as inventoryRepo from './inventoryRepo.js';
...
export function mountCommerceApi(app) {
  ...
  // ---- Store locations ----
  router.get('/locations', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    res.json(await locationsRepo.listLocations(orgIdFor(req)));
  });
  router.post('/locations', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    res.json({ success: true, location: await locationsRepo.createLocation(orgIdFor(req), req.body || {}) });
  });
  router.patch('/locations/:id', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const location = await locationsRepo.updateLocation(orgIdFor(req), req.params.id, req.body || {});
    if (!location) return res.status(404).json({ error: 'Location not found' });
    res.json({ success: true, location });
  });
  router.delete('/locations/:id', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const removed = await locationsRepo.deleteLocation(orgIdFor(req), req.params.id);
    if (!removed) return res.status(404).json({ error: 'Location not found' });
    res.json({ success: true });
  });

  // ---- Staff / sellers (floor staff, name-only) ----
  router.get('/staff', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    res.json(await staffRepo.listStaff(orgIdFor(req)));
  });
  router.post('/staff', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    res.json({ success: true, staff: await staffRepo.createStaff(orgIdFor(req), req.body || {}) });
  });
  router.patch('/staff/:id', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const staff = await staffRepo.updateStaff(orgIdFor(req), req.params.id, req.body || {});
    if (!staff) return res.status(404).json({ error: 'Staff not found' });
    res.json({ success: true, staff });
  });
  router.delete('/staff/:id', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const removed = await staffRepo.deleteStaff(orgIdFor(req), req.params.id);
    if (!removed) return res.status(404).json({ error: 'Staff not found' });
    res.json({ success: true });
  });

  // Combined seller options for the manual-sale form: CMS Team members + the
  // name-only floor staff list.
  router.get('/sellers', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const orgId = orgIdFor(req);
    const [staff, team] = await Promise.all([
      staffRepo.listStaff(orgId).catch(() => []),
      storage.team.list(orgId).catch(() => []),
    ]);
    res.json({
      staff: staff.map((s) => ({ id: s.id, name: s.name })),
      teamMembers: (team || []).map((t) => ({ id: t.id || t.email, name: t.name || t.email, email: t.email })),
    });
  });

  // ---- Inventory (per product x location) ----
  router.get('/inventory', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    res.json(await inventoryRepo.listInventory(orgIdFor(req)));
  });
  router.patch('/inventory', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const { productId, locationId, quantity } = req.body || {};
    if (!productId || !locationId) return res.status(400).json({ error: 'productId and locationId are required' });
    res.json({ success: true, entry: await inventoryRepo.setQuantity(orgIdFor(req), productId, locationId, quantity) });
  });

  // ---- Manual / in-store order ----
  router.post('/orders', requireCommerceTier('admin'), requireOrgCtx, async (req, res) => {
    const orgId = orgIdFor(req);
    const { items = [], locationId = null, soldBy = null, customerEmail = null, status = 'paid' } = req.body || {};
    if (!Array.isArray(items) || items.length === 0) return res.status(400).json({ error: 'At least one line item is required.' });
    const clean = items
      .filter((i) => i.productId && Number(i.quantity) > 0)
      .map((i) => ({ productId: i.productId, variantId: null, name: i.name || 'Item', price: Number(i.price) || 0, quantity: Math.round(Number(i.quantity)) }));
    if (clean.length === 0) return res.status(400).json({ error: 'No valid line items.' });
    const total = clean.reduce((s, i) => s + i.price * i.quantity, 0);
    const order = await ordersRepo.createOrder({
      org_id: orgId, customer_email: customerEmail || null, items: clean, total,
      status: ['pending', 'paid', 'refunded', 'cancelled'].includes(status) ? status : 'paid',
      location_id: locationId, sold_by: soldBy, channel: 'manual',
    });
    // Draw down stock at the sale's location (best-effort; never fails the sale).
    if (locationId) {
      for (const it of clean) await inventoryRepo.adjustQuantity(orgId, it.productId, locationId, -it.quantity).catch(() => {});
    }
    res.json({ success: true, order });
  });
