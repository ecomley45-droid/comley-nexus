...
export function mountViewAsHandoff(app) {
  app.post('/api/view-as/accept', parseForm, async (req, res) => {
    ...
    // Single-use: claimViewAsTokenUse (lib/coreAppAccess.js) claims the
    // token's jti in Core's view_as_token_uses table (atomic insert; a
    // conflict means replay) -- replaces the old in-memory usedNonces Map,
    // which only caught a replay within one serverless instance and missed
    // it across Vercel's multiple instances.
