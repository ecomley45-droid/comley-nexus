...
import { htmlToTypedBlocks } from '../src/shared/htmlToBlocks.js';
...
export function cleanTheme(raw) {
  ...
  // All three type roles, each validated against the same closed key map, so
  // an unknown family is dropped rather than reaching buildThemeStyleBlock.
  for (const key of ['fontFamily', 'fontDisplay', 'fontMono']) {
    if (FONT_STACKS[raw[key]]) theme[key] = raw[key];
  }
...
export function summarizePayload(payload) {
  ...
  for (const p of pages) {
    if (p.editorMode === 'full-html') {
      // These install as typed blocks too (see materializeInstall), so count
      // the blocks the document classifies into rather than reporting zero.
      fullHtmlPages += 1;
      sectionCount += htmlToTypedBlocks(p.fullHtml).length;
      continue;
    }
...
export function materializeInstall(payload, { stamp = Date.now() } = {}) {
  ...
  const outPages = pages.map((p, pi) => {
    // A template page stored as one hand-written document installs as TYPED
    // blocks -- classified block types with extracted fields -- not as a
    // locked full-code page and not as raw-HTML modules. That's what makes
    // every template no-code editable: a card grid arrives as a card-grid
    // block with its cards in `items`, editable one field at a time.
    //
    // The trade is explicit: typed blocks re-render through Nexus's own
    // renderers, so the source document's CSS no longer applies and the
    // content is re-skinned to the workspace theme. `fullHtml` is still
    // written to the page unchanged, so nothing is lost -- Convert (see
    // src/cms/lib/pageModes.js) hands back a pixel-perfect full-code copy of
    // the original whenever the original design matters more.
    if (p.editorMode === 'full-html') {
      const typed = htmlToTypedBlocks(p.fullHtml);
      return {
        ...base(pi), name: p.name, slug: p.slug,
        content: typed.map((s, si) => ({
          id: `sec-${stamp}-${pi}-${si}`,
          name: s.name,
        ...
        editorMode: 'blocks', fullHtml: p.fullHtml,
        // The document carries its own header/footer, so re-adding the
        // workspace globals on top would double them up.
...
const SEED_META = {
  ...
  realtor: {
    category: 'Services',
    features: [
      'A four-question qualifying form instead of a contact box',
      'Pages for every nav item and call to action',
      'Trades index, homeowner-portal teaser and a career timeline',
      'Paper-and-ink editorial theme with real web fonts',
    ],
  },
  'local-service': {
    category: 'Services',
