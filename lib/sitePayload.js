...
import { splitHtmlIntoSections } from '../src/shared/htmlSections.js';
...
export function summarizePayload(payload) {
  ...
  for (const p of pages) {
    if (p.editorMode === 'full-html') {
      // These install as blocks too (see materializeInstall), so count the
      // modules the document will split into rather than reporting zero.
      fullHtmlPages += 1;
      sectionCount += splitHtmlIntoSections(p.fullHtml).length;
      continue;
    }
...
export function materializeInstall(payload, { stamp = Date.now() } = {}) {
  ...
  const outPages = pages.map((p, pi) => {
    // A template page stored as one hand-written document installs as
    // editable blocks, not as a locked full-code page: every top-level
    // module in the document becomes its own block (design intact -- the
    // document's stylesheets ride along in a leading "Page styles" block),
    // so a workspace can reorder, delete and restyle the parts of a template
    // instead of editing raw HTML or nothing at all.
    //
    // `fullHtml` is still written to the page, unchanged. Nothing is lost:
    // the original document stays on the page, and Convert (see
    // src/cms/lib/pageModes.js) turns it back into a full-code copy.
    if (p.editorMode === 'full-html') {
      const sections = splitHtmlIntoSections(p.fullHtml, { makeId: (i) => `sec-${stamp}-${pi}-${i}` });
      return {
