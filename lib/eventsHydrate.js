...
import { EVENT_BOUND_TYPES, applyEventsToFields, expandRecurring, accentWrap } from '../src/shared/eventsMap.js';
import { events as eventsStore, calendars as calendarsStore } from './eventsStore.js';
...
export async function hydrateEventBlocks(page, orgId, timeZone) {
  ...
  // Fetch each distinct calendar selection once, plus the calendar colors.
  const cache = new Map();
  ...
  const colorById = Object.fromEntries((await calendarsStore.list(orgId).catch(() => [])).map((c) => [c.id, c.color]));
  ...
  page.content = await Promise.all(page.content.map(async (section) => {
    ...
    const evts = expandRecurring(await fetchFor(calId));
    const fields = applyEventsToFields(section.blockType, section.fields, evts, { timeZone });
    let html = renderBlock(section.blockType, fields) || section.html;
    const color = calId !== 'all' ? colorById[calId] : null;
    if (color) html = accentWrap(html, color);
    return { ...section, html };
  }));
  return page;
}
