...
export const media = {
  ...
  async get(orgId, id) {
    ...
    const { data, error } = await db().from('media')
      .select('id, name, filename, mime_type, url').eq('org_id', orgId).eq('id', id).maybeSingle();
...
export const orgs = {
  async list() {
    const { data, error } = await db().from('orgs')
      .select('id, name, domain, plan, feature_flags, created_at')
      .order('created_at', { ascending: true });
  ...
  async get(id) {
    const { data, error } = await db().from('orgs')
      .select('id, name, domain, plan, feature_flags, created_at').eq('id', id).maybeSingle();
...
// Returns { id, slug, name, role, feature_flags } or null if none.
export async function orgForUser(email) {
  if (!email) return null;
  const { data, error } = await db()
    .from('org_members')
    .select('role, orgs!inner(id, name, feature_flags)')
    .eq('user_email', email)
    .maybeSingle();
  ...
  return {
    ...
    name: data.orgs.name,
    role: data.role,
    feature_flags: data.orgs.feature_flags || {},
  };
}
