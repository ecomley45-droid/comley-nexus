...
//
// CMS never SIGNS one of THESE tokens (only Command issues view-as grants),
// so only the verify half lives here for that pair. (Below, a second,
// unrelated pair — signSharedSessionCookie/verifySharedSessionCookie — DOES
// sign, for a different purpose: see that section's own header.)
...
// SINGLE-USE, 60s-TTL handoff token that Command signs and CMS only ever
// verifies once (via claimViewAsTokenUse) before discarding it. The pair
// below SIGNS and verifies CMS's own long-lived `shared_session` cookie —
...
// SESSION handoff (see lib/sessionHandoff.js) a LASTING sign-in on CMS,
...
// one-time grant CMS never signs; a signSharedSessionCookie payload is a
// standing identity claim CMS signs and re-checks itself, every request,
// for weeks.
