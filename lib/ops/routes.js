...
export function mountOpsApi(app) {
  ...
  // ---- Feedback assignment ----------------------------------------------
  app.patch('/api/feedback/:id/assignee', requireOrg, requirePermission('feedback', 'edit'), async (req, res, next) => {
    try {
      const orgId = req.org.id;
  ...
  app.patch('/api/feedback/:id/system', requireOrg, requirePermission('feedback', 'edit'), async (req, res, next) => {
    try {
  ...
  const projectPrefs = (p) => ({
    ...
    dismissed_dashboard_tasks: p.dismissed_dashboard_tasks || [],
    bio: p.bio ?? null,
