...
// Viewer identity comes from X-User-* headers set by the client (see
// src/cms/lib/api.js). Falls back to a local dev identity so single-user
// setups don't need to plumb Clerk in yet.
const viewer = (req) => ({
  email: req.header('X-User-Email') || 'local@comley-builder',
  name: req.header('X-User-Name') || 'Local Dev',
...
const requireRole = (minRole) => (req, res, next) => {
  if ((ROLE_RANK[req.userRole] ?? 0) < ROLE_RANK[minRole]) {
    return res.status(403).json({ error: `Requires ${minRole} role (you are ${req.userRole})` });
  }
  next();
};
...
export function mountOpsApi(app) {
  ...
  app.patch('/api/user/preferences', (req, res) => {
    ...
    // Shallow merge; `integrations` merges a level deeper so callers can
    // toggle one key at a time without wiping the rest.
    const next = { ...existing };
    ...
    res.json({
      view: next.view ?? null,
      detail_mode: next.detail_mode ?? null,
      schedule_order: next.schedule_order ?? null,
      work_start: next.work_start ?? null,
      work_end: next.work_end ?? null,
      work_days: next.work_days ?? null,
