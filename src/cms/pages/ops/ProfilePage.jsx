    ...
    <div className="mt-3 rounded-lg border border-white/10 bg-white/[0.02] p-3 grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      <div>
        ...
        <input
          type="range"
          min={8000}
          max={provider.maxContext}
          step={8000}
          value={contextWindow}
          onChange={(e) => onChange({ context_window: Number(e.target.value) })}
          className="w-full accent-glass-indigo"
        />
      </div>
      <div>
        <label className="text-[10px] uppercase tracking-wide text-zinc-500 block mb-1">
      ...
      <div className="md:col-span-2">
        ...
        <GlassTextarea
          ...
          rows={2}
          placeholder={`Personality / house style for ${provider.label}…`}
          className="text-sm w-full"
        />
      </div>
    </div>
          ...
          return (
            <li key={int.id} className={`py-3.5 ${i > 0 ? 'border-t border-white/10' : ''}`}>
              ...
              {showAi && (
                <AiSettingsPanel
                  providerId={int.id}
                  value={aiSettings[int.id]}
                  onChange={(patch) => patchAiSettings(int.id, patch)}
                />
              )}
            </li>
          );
